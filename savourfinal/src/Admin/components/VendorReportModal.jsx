...
const generatePDF = async (vendor) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/admin/orders/${vendor}`
    );
    const data1 = response.data;
    console.log(data1);
    const doc = new jsPDF();
    doc.setProperties({
      title: `Orders Report for Vendor`,
    });

    let y = 20;
    doc.setFontSize(18);
    doc.text(`Order Report for ${vendor}`, 105, y, { align: "center" });
    y += 10;
    ...
    <Modal open={true} onClose={onClose} aria-labelledby="vendor-report-modal">
      ...
      >
        ...
        <Typography variant="subtitle1">
          Delivered Orders: {placedOrdersCount}
        </Typography>
        <Typography variant="subtitle1">
          Completed Orders: {acceptedOrdersCount}
        </Typography>
