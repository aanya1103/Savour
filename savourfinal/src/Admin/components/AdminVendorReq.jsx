...
const VendorRequest = () => {
  ...
  const getAllVendors = async () => {
    try {
      const resp = await fetch("http://localhost:5000/admin/vrequest", {
        method: "GET",
      });
  ...
  const handleTickClick = async (vendorId) => {
    try {
      ...
      const resp = await fetch(
        `http://localhost:5000/admin/vrequest/${vendorId}`,
        {
          method: "GET",
        }
      );
      ...
      const confirmResp = await fetch(
        "http://localhost:5000/admin/vrequest/confirm",
        {
          method: "POST",
          headers: {
  ...
  const handleCrossClick = async (vendorId) => {
    try {
      await fetch("http://localhost:5000/admin/vrequest/reject", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
