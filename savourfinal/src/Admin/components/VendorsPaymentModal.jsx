...
const VendorsPaymentModal = ({ vendor, onClose }) => {
  ...
  const handlePayClick = async () => {
    try {
      const response = await fetch(`http://localhost:5000/admin/payout`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <Modal open={true} onClose={onClose} aria-labelledby="vendor-report-modal">
      ...
      >
        <Typography variant="h5" style={{ marginBottom: "20px" , color:"black"}}>
        ...
        {loading ? (
          ...
          <>
            ...
            <Typography variant="body1">
              <strong>Vendor's Income:</strong> Rs. {amt-amt*(5/100)}
            </Typography>
          </>
        )}
        <Box
